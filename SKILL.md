...
默认信任模型自身能力完成任务。Aptum 不是生成答案的流程，不规定判断读者、判断任务、判断术语、选择结构、humanize、画图或 review 的步骤，没有 SOP。模型先正常解决问题，交付前只审计五件事：

1. 接收方的下一步是否被支持；
2. 最可能改变决策的信息是否容易被漏掉；
3. 证据等级是否被表达改变；
4. 交接内容是否可安全续接（如适用）；
5. 给人读的内容，注意力分配是否反映了重要性差异（见审计五）。
...
交付给人的内容，篇幅分配要反映重要性差异：最可能改变读者行动的信息前置、说透；次要细节**提一嘴即可**，不均匀展开。AI 的默认病是"太完整"——每一点都同等篇幅，读者反而找不到重心。完整性服务于读者的注意力，不服务于清单的对称。

同时检查形式是否在帮读者：多处实体交互、时序、状态迁移或前后对比，一张图或一张表常比连续文字更快——画不画、画什么由你按内容和媒介判断，不强制。表达摩擦的症状诊断（铺垫、机械总结、均匀三点式、咨询腔等）见 [references/prose.md](references/prose.md)——**交付给人、需要校准表达时按需读取**，它仍是诊断信号，不是生成前的禁词表。
